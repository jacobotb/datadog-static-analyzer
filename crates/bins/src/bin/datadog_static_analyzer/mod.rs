...
use cli::rule_utils::{
    check_rules_checksum, convert_secret_result_to_rule_result, count_violations_by_severities,
    get_languages_for_rules, get_rulesets_from_file,
};
...
use kernel::analysis::analyze::{generate_flow_graph_dot, AnalysisEngine};
...
pub fn declare_options() -> Options {
...
pub fn parse_options(opts: Options) -> Result<(Matches, CliConfiguration)> {
    ...
    Ok((matches, configuration))
}

pub fn run_analysis(
    configuration: CliConfiguration,
    analysis_engine: impl AnalysisEngine + Sync,
) -> Result<()> {
    print_configuration(&configuration);

    let directory_path = Path::new(&configuration.source_directory);
    ...
    if configuration.should_verify_checksum {
    ...
    let files_in_repository = get_files(
        ...
        configuration.source_subdirectories.clone(),
        &configuration.path_config,
    )
    .expect("unable to get the list of files to analyze");

    // check if we do a diff-aware scan
    let diff_aware_parameters: Option<DiffAwareData> = if configuration.diff_aware_requested {
    ...
    for language in &languages {
        ...
        let rules_for_language = configuration
            .rules
            .iter()
                ...
                |(mut stats, mut fold_results), path| {
                    ...
                    let res = if let Ok(file_content) = fs::read_to_string(&path) {
                        ...
                        let mut results = analysis_engine
                            ...
                            .unwrap_or_else(|e| {
                                eprintln!("error analyzing file {}: {}", &relative_path, e);
                                vec![]
                            });
                        ...
                        if configuration.debug_java_dfa && *language == Language::Java {
    ...
    if configuration.secrets_enabled {
        ...
        let sds_scanner = build_sds_scanner(&configuration.secrets_rules);

        let nb_secrets_rules: usize = configuration.secrets_rules.len();
    ...
    if configuration.print_violations && nb_violations > 0 {
    ...
    let fail_on_violations = !configuration.fail_any_violation_severities.is_empty()
        && count_violations_by_severities(
            &all_rule_results,
            &configuration.fail_any_violation_severities,
        ) > 0;
    ...
    let value = match configuration.output_format {
        ...
        OutputFormat::Sarif => generate_sarif_file(
            ...
            secrets_results,
            SarifReportMetadata {
                add_git_info: configuration.add_git_info,
...
mod tests {
    ...
    use crate::datadog_static_analyzer::{
        Aggregate, AnalysisStatistics, FileName, STATS_MAX_PARSE_TIMES,
    };
